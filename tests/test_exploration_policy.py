...
from source_scout import deepseek, fastcontext, fastcontext_routing, server
...
def test_committed_default_and_codex_override(monkeypatch):
    ...
    assert "source_scout" not in config["mcp_servers"]
    plugin = json.loads((root / "plugins/source-scout/.mcp.json").read_text())
    scout = plugin["mcpServers"]["source_scout"]
    assert set(scout["enabled_tools"]) == set(server.DEFAULT_MCP_TOOL_NAMES)
...
async def test_allowed_routes_and_journal(monkeypatch, tmp_path, mode, use_case, attempts):
    ...
    def handler(request):
        ...
        return httpx.Response(
            200, json=_response_tool_call_json("Read", {"path": "a.py", "offset": 1, "limit": 2})
        )

    reason = "  Local references leave the return contract unclear  "
    result = await fastcontext.explore_local_project(
        "Trace actual_symbol's caller contract",
        tmp_path,
        max_turns=1,
        reason=reason,
        use_case=use_case,
        attempted_local_methods=attempts,
        transport=httpx.MockTransport(handler),
    )
    assert len(calls) == 1 and result.stop_reason == "call_budget"
    report = json.loads(Path(result.report_path).read_text())
    assert report["report_schema_version"] == "source-scout-investigation-v3"
